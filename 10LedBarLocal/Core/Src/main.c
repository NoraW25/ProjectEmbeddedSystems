...
void LEDBar_CO2(int co2){
	uint8_t co2Data [2]; //Om 2 bytes te sturen via I^2c.

	if (co2<=400)//bij een c02 niveau lager dan 400 worden 0 leds aangezet. 
	{
	    co2Data[1]=0x00;
	    co2Data[0]=GPIOA_REG;
	...
	else if(co2>400&&co2<=1760)//bij een co2 niveau tussen de 400 en 1760 gaan alle groene leds aan
	{
		co2Data[1]=0xFF;
		co2Data[0]=GPIOA_REG;
		HAL_I2C_Master_Transmit(&hi2c1, MCP23017_ADDR, co2Data, 2, HAL_MAX_DELAY);
	...
	else if(co2>1760&&co2<=1840)//bij een co2 niveau groter dan 1760 en de 1840 gaat de oranje led aan.
	{
		co2Data[1]=0x00;
		co2Data[0]=GPIOA_REG;
		HAL_I2C_Master_Transmit(&hi2c1, MCP23017_ADDR, co2Data, 2, HAL_MAX_DELAY);
	...
	else{//bij een co2 niveau groter dan 1840 gaat de rode led aan. 
...
void LEDBar_Temp(double temp){
	...
	if (temp<=16.00)//Bij een temperatuur van 16 graden of lager gaat er 1 groene led aan
	{
		tempData[1]=0x01;
	...
	else if(temp>16.00&&temp<=21.00)//Bij een temperatuur van hoger dan 16 graden en lager
  //of gelijk aan 21 graden, gaan er 8 groene leds aan. 
	{
		tempData[1]=0xFF;
		tempData[0]=GPIOA_REG;
		HAL_I2C_Master_Transmit(&hi2c1, MCP23017_ADDR, tempData, 2, HAL_MAX_DELAY);
	...
	else if(temp>21.00&&temp<=23.00)//Bij een hogere temperatuur dan 21 graden en kleiner of gelijk aan
  //23 graden gaat de oranje led aan. 
	{
		tempData[1]=0x00;
		tempData[0]=GPIOA_REG;
		HAL_I2C_Master_Transmit(&hi2c1, MCP23017_ADDR, tempData, 2, HAL_MAX_DELAY);
	...
	else{//bij een temperatuur hoger dan 23 graden, gaat de rode led aan. 
...
void LEDBar_Humidity(int humidity){
	...
	if (humidity<=20)//Bij een humidity van 20 of lager gaan er geen leds aan. 
	{
		humidityData[1]=0x00;
  ...
  //Bij een humidity groter dan 20 en kleiner of gelijk aan 60 gaan er 8 groene ledjes aan. 
	{
		humidityData[1]=0xFF;
		humidityData[0]=GPIOA_REG;
		HAL_I2C_Master_Transmit(&hi2c1, MCP23017_ADDR, humidityData, 2, HAL_MAX_DELAY);
  ...
  //Bij een humidity groter dan 60 en kleiner of gelijk aan 79 gaat er 1 oranje led aan. 
	{
		humidityData[1]=0x00;
		humidityData[0]=GPIOA_REG;
		HAL_I2C_Master_Transmit(&hi2c1, MCP23017_ADDR, humidityData, 2, HAL_MAX_DELAY);
    ...
    //Bij een humidity die groter is dan 80 gaat er 1 rode led aan. 
